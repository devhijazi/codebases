...
export interface ScheduleCanceledTopicPayload {
  scheduleId: string;
}

export class ScheduleCanceledTopic extends Topic {
  async execute(message: KafkaMessage): Promise<void> {
    ...
    const payload = <ScheduleCanceledTopicPayload>JSON.parse(messageValue);
